...
public class PG_001_LoginPage {
    ...
    public PG_003_Dashboard VerifyToastMessage(String message) {
        ...
        return new PG_003_Dashboard(TestContext.getDriver());
    }

    public PG_011_ChatBotAI click_on_ChatBot_Menu() {
        String methodName = Thread.currentThread().getStackTrace()[1].getMethodName().replace("_", " ");
        try {
            TestContext.getWait().until(ExpectedConditions.visibilityOf(ChatBot)).click();
            Thread.sleep(2000); // Avoid using Thread.sleep; instead use WebDriverWait
            TestContext.getJsExecutor().executeScript("window.scrollTo(0, document.body.scrollHeight);");
            ExtentReportManager.reportStep(methodName, "pass");
            TestContext.getLogger().info(methodName);
        } catch (Exception e) {
            TestContext.getLogger().error(methodName + " " + "Is not clicked");
            e.printStackTrace();
        }
        return new PG_011_ChatBotAI(TestContext.getDriver());
    }
}
